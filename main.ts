...
for (let i = 0; i < predictions.length; i++) {
  ...
  console.log(
    `Is car[${item.index}] == '${item.value}'? I predict ${item.prediction}.`
  );
...
new_user_name.forEach((new_user_name) => {
  if (
    current_user_name.some(
      (current_user_name) =>
        current_user_name.toLocaleLowerCase() ===
...
function make_car(manufacturer: string, modelName: string, options: {[key: string]: any}): Car {
